...
\subsection[Shared Everything/\-Non-Partitioned]{Shared Everything/\-Non-Partitioned (SE/NP)}
 ...
 \begin{frame}
	...
	\only<2>{%
		\resizebox{!}{.97\architecturesheight}{%
			\seDiagram%
		}%
	}%
\end{frame}
 ...
 \begin{frame}
	...
	\begin{itemize}
		\visible<2->{\item	metadata (incl. locks) aren't partitioned}
		\visible<3->{\item[$\rightarrow$]	physical synchronization (latches, atomic instructions) required}
		\visible<4->{\item	data and indices aren't partitioned}
		\visible<5->{\item[$\rightarrow$]	logical synchronization using a concurrency control protocol also required}
		\visible<6->{\item	transactions completely executed by one thread}
		\visible<6->{\item	thread-assignment depends only on load}
	\end{itemize}
\end{frame}
 ...
 \begin{frame}[fragile]
	...
	\begin{itemize}
		\visible<2->{\item[$+$]	no partitioning required (e.g. manual selection of a partitioning strategy)}
		\visible<3->{\item[$+$]	partitioning would be sensitive to the workload}
		\visible<4->{\item[$+$]	changed workloads would require (probably expensive) repartitioning to benefit from partitioning}
		\visible<5->{\item[$-$]	each thread might access every record at arbitrary times}
			\begin{itemize}
				\visible<17->{\item[$-$]	each CPU cache may contain any part of the data \bm{$\rightarrow$} cache pollution}
				\visible<19->{\item[$-$]	each CPU may access any part of the data \bm{$\rightarrow$} data movement between NUMA regions}
				\visible<21->{\item[$-$]	each CPU may acquire any latch \bm{$\rightarrow$} data movement between NUMA regions}
				\visible<22->{\item[$-$]	each CPU may atomically write to any semaphore \bm{$\rightarrow$} hardware cache coherence overhead}
			\end{itemize}
	\end{itemize}
	...
	\begin{tikzpicture}[remember picture, overlay]
		\node[visible on = <6-16>, shape = rectangle, draw = black, fill = black!0] at (current page.center) {%
			\begin{adjustbox}{width = \enhancedtextwidth}%
				\begin{tikzpicture}
					\begin{axis}[xlabel = {TPC-C runtime $\left[\si{\second}\right]$},
						...
						\addplot[visible on = <7->, only marks, mark = x, draw = black] table {data/record_accesses_per_thread_conventional_0.csv};
						\addplot[visible on = <8->, only marks, mark = x, draw = cyan] table {data/record_accesses_per_thread_conventional_1.csv};
						\addplot[visible on = <9->, only marks, mark = x, draw = red] table {data/record_accesses_per_thread_conventional_2.csv};
						\addplot[visible on = <10->, only marks, mark = x, draw = green] table {data/record_accesses_per_thread_conventional_3.csv};
						\addplot[visible on = <11->, only marks, mark = x, draw = blue] table {data/record_accesses_per_thread_conventional_4.csv};
						\addplot[visible on = <12->, only marks, mark = x, draw = black!50] table {data/record_accesses_per_thread_conventional_5.csv};
						\addplot[visible on = <13->, only marks, mark = x, draw = cyan!50] table {data/record_accesses_per_thread_conventional_6.csv};
						\addplot[visible on = <14->, only marks, mark = x, draw = red!50] table {data/record_accesses_per_thread_conventional_7.csv};
						\addplot[visible on = <15->, only marks, mark = x, draw = green!50] table {data/record_accesses_per_thread_conventional_8.csv};
						\addplot[visible on = <16->, only marks, mark = x, draw = blue!50] table {data/record_accesses_per_thread_conventional_9.csv};
					\end{axis}
				\end{tikzpicture}%
			\end{adjustbox}%
    												   };
		\node[visible on = <18>, shape = rectangle, draw = black, fill = black!0] at (current page.center) {%
		...
		\node[visible on = <20>, shape = rectangle, draw = black, fill = black!0] at (current page.center) {%
			\begin{adjustbox}{width = \enhancedtextwidth}%
				\begin{tikzpicture}
...
\end{frame}
 ...
 \begin{frame}
	...
	\begin{itemize}
		\visible<2->{\item	metadata (incl. locks) are physically partitioned}
		\visible<3->{\item[$\rightarrow$]	no physical synchronization (latches, atomics) required}
		\visible<4->{\item	data and indices are logically partitioned}
		\visible<5->{\item[$\rightarrow$]	logical synchronization using a concurrency control protocol only locally required}
		\visible<6->{\item	threads are assigned to data}
		\visible<6->{\item	transactions migrate to threads owning the accessed data}
	\end{itemize}
\end{frame}
...
\end{frame}
 ...
 \begin{frame}
	\frametitle{Pros \& Cons of DORA I}
	
	\begin{itemize}
		\visible<2->{\item[$+$]	each thread accesses only the records of its partition}
			\begin{itemize}
				\visible<4->{\item[$+$]	each CPU cache may contain only data of its partition \bm{$\rightarrow$} lower cache pollution}
				\visible<5->{\item[$+$]	each CPU may access only data of its partitions \bm{$\rightarrow$} no data movement between NUMA regions (for single-CPU transactions)}
				\visible<6->{\item[$\rightarrow$]	No physical synchronization required!}
			\end{itemize}
		\visible<7->{\item[$+$]	logical partitioning allows fast repartitioning when the workload changes}
		\visible<8->{\item[$+$]	multi-site transactions could exploit intra-transaction parallelism}
		\visible<9->{\item[$-$]	partitioning required (e.g. manual selection of a partitioning strategy---\textit{called routing rule})}
	\end{itemize}
	
	\begin{tikzpicture}[remember picture, overlay]
		\node[visible on = <3>, shape = rectangle, draw = black, fill = black!0] at (current page.center) {%
			\begin{adjustbox}{width = \enhancedtextwidth}%
				\begin{tikzpicture}
					\begin{axis}[xlabel = {TPC-C runtime $\left[\si{\second}\right]$},
							  xlabel near ticks,
...
\end{frame}
 ...
 \begin{frame}
	\frametitle{Pros \& Cons of DORA II}
	
	\begin{itemize}
		\visible<2->{\item[$-$]	partitioning is sensitive to the workload}
		\visible<3->{\item[$-$]	multi-site transactions require expensive fiber-migration (probably between NUMA regions)}
		\visible<4->{\item[$-$]	accessed partitions need to be calculated during query analysis for optimal performance \bm{$\rightarrow$} slower accesses with secondary index}
		\visible<5->{\item[$-$]	primary index is shared \bm{$\rightarrow$} centralized latching for inserts/deletes still required \bm{$\rightarrow$} some contention on the shared latch}
		\visible<6->{\item[$-$]	centralized deadlock detection still required (for \lstinline{DL\_DETECT})}
	\end{itemize}
\end{frame}
...
\end{frame}
 ...
 \begin{frame}
	...
	\begin{itemize}
		\visible<2->{\item	metadata (incl. locks) are physically partitioned}
		\visible<3->{\item[$\rightarrow$]	no physical synchronization (latches, atomics) required}
		\visible<4->{\item	data and indices are physically partitioned}
		\visible<5->{\item[$\rightarrow$]	logical synchronization using a concurrency control protocol only locally required}
		\visible<6->{\item	transactions completely executed by one thread (ideally the one owning the majority of the records)}
		\visible<7->{\item	thread accesses remote records by passing messages to the threads owning them}
						\begin{itemize}
							\visible<8->{\item	message passing implemented using shared variables}
							\visible<9->{\item	remote records are passed as pointers}
						\end{itemize}
	\end{itemize}
\end{frame}

{
	\usebackgroundtemplate{%
		...
		\resizebox{\linewidth}{!}{%
			\begin{tikzpicture}
				\begin{umlseqdiag}
					\umlactor[no ddots]		{Application 0}
					\umlactor[no ddots]		{Application 1}
				
					\umlobject[no ddots]		{Thread 0}
					\umlobject[no ddots, /tikz/xshift = 2em]		{Thread 1}
					
					\only<2-18>{\begin{umlcall}[op = {$BOT$}]{Application 0}{Thread 0}
						\only<3->{\begin{umlcall}[op = {$read\left(txn = 0, RID = 0\right)$}, return = {$x_0$}]{Thread 0}{Thread 0}
						\end{umlcall}}
						\only<4->{\begin{umlcall}[op = {$read\left(txn = 0, RID = 1\right)$}, return = {$x_1$}]{Thread 0}{Thread 0}
						\end{umlcall}}
						\only<5->{\begin{umlcall}[op = {$enqueue\left(read\left(0, 3\right)\right)$}, type = asynchron]{Thread 0}{Thread 1}
							\only<6->{\begin{umlcall}[op = {$dequeue\left(\right)$}, return = {$read\left(txn = 0, RID = 3\right)$}]{Thread 1}{Thread 1}
							\end{umlcall}}
							\only<7->{\begin{umlcall}[op = {$read\left(txn = 0, RID = 3\right)$}, return = {$x_3$}]{Thread 1}{Thread 1}
							\end{umlcall}}
							\only<10->{\begin{umlcall}[op = {$enqueue\left(x_3)\right)$}, type = asynchron]{Thread 1}{Thread 0}
							\end{umlcall}}
						\end{umlcall}}
						\only<13->{\begin{umlcall}[op = {$dequeue\left(\right)$}, return = {$x_3$}, dt = 10]{Thread 0}{Thread 0}
						\end{umlcall}}
						\only<14->{\begin{umlcall}[op = {$commit\left(txn = 0\right)$}]{Thread 0}{Thread 0}
							\only<15->{\begin{umlcall}[op = {$enqueue\left(commit\left(0\right)\right)$}, type = asynchron, return = {$enqueue\left(true\right)$}]{Thread 0}{Thread 1}
								\only<16->{\begin{umlcall}[op = {$dequeue\left(\right)$}, return = {$commit\left(txn = 0\right)$}]{Thread 1}{Thread 1}
								\end{umlcall}}					
								\only<17->{\begin{umlcall}[op = {$commit\left(txn = 0\right)$}, return = {$true$}]{Thread 1}{Thread 1}
								\end{umlcall}}
								\only<18->{\begin{umlcall}[op = {$enqueue\left(true\right)$}, type = asynchron]{Thread 1}{Thread 0}
								\end{umlcall}}
							\end{umlcall}}
						\end{umlcall}}
					\end{umlcall}}
					
					\only<8-11>{\begin{umlcall}[op = {$BOT$}, dt = 31]{Application 1}{Thread 0}
						\only<9->{\begin{umlcall}[op = {$read\left(txn = 0, RID = 0\right)$}, return = {$x_0$}]{Thread 0}{Thread 0}
						\end{umlcall}}
						\only<11->{\begin{umlcall}[op = {$commit\left(txn = 0\right)$}, return = {$true$}, dt = 5]{Thread 0}{Thread 0}
						\end{umlcall}}
					\end{umlcall}}

					\only<19>{\begin{umlcall}[op = {$BOT$}]{Application 0}{Thread 0}
						\only<3->{\begin{umlcall}[op = {$read\left(txn = 0, RID = 0\right)$}, return = {$x_0$}]{Thread 0}{Thread 0}
						\end{umlcall}}
						\only<4->{\begin{umlcall}[op = {$read\left(txn = 0, RID = 1\right)$}, return = {$x_1$}]{Thread 0}{Thread 0}
						\end{umlcall}}
						...
						\only<13->{\begin{umlcall}[op = {$dequeue\left(\right)$}, return = {$x_3$}, dt = 10]{Thread 0}{Thread 0}
						\end{umlcall}}
						\only<14->{\begin{umlcall}[op = {$commit\left(txn = 0\right)$}, return = {$true$}]{Thread 0}{Thread 0}
							\only<15->{\begin{umlcall}[op = {$enqueue\left(commit\left(0\right)\right)$}, type = asynchron, return = {$enqueue\left(true\right)$}]{Thread 0}{Thread 1}
								\only<16->{\begin{umlcall}[op = {$dequeue\left(\right)$}, return = {$commit\left(txn = 0\right)$}]{Thread 1}{Thread 1}
								\end{umlcall}}					
								\only<17->{\begin{umlcall}[op = {$commit\left(txn = 0\right)$}, return = {$true$}]{Thread 1}{Thread 1}
					...
					\only<20>{\begin{umlcall}[op = {$BOT$}, return = {$EOT$}]{Application 0}{Thread 0}
						\only<3->{\begin{umlcall}[op = {$read\left(txn = 0, RID = 0\right)$}, return = {$x_0$}]{Thread 0}{Thread 0}
						\end{umlcall}}
						\only<4->{\begin{umlcall}[op = {$read\left(txn = 0, RID = 1\right)$}, return = {$x_1$}]{Thread 0}{Thread 0}
						\end{umlcall}}
					...
					\only<12->{\begin{umlcall}[op = {$BOT$}, return = {$EOT$}, dt = 31]{Application 1}{Thread 0}
						\only<9->{\begin{umlcall}[op = {$read\left(txn = 0, RID = 0\right)$}, return = {$x_0$}]{Thread 0}{Thread 0}
						\end{umlcall}}
						\only<11->{\begin{umlcall}[op = {$commit\left(txn = 0\right)$}, return = {$true$}, dt = 5]{Thread 0}{Thread 0}
						\end{umlcall}}
					\end{umlcall}}
				\end{umlseqdiag}
				\node[draw = none]	at (17.5, 0)	{};
			\end{tikzpicture}%
		}
		\end{minipage}
	}
	 \begin{frame}
		\frametitle{\colorbox{white}{Example}}
 ...
 \begin{frame}
	\frametitle{Pros \& Cons of Delegation}
	
	\begin{itemize}
		\visible<2->{\item[$+$]	only multi-site transactions cause a thread to access unowned records}
			\begin{itemize}
				\visible<3->{\item[$+$]	each CPU cache usually only contains data of its partition \bm{$\rightarrow$} lower cache pollution}
				\visible<4->{\item[$+$]	each CPU usually only accesses data of its partitions \bm{$\rightarrow$} fewer data movement between NUMA regions}
				\visible<5->{\item[$\rightarrow$]	No physical synchronization required!}
			\end{itemize}
		\visible<6->{\item[$+$]	message-passing required for multi-site transactions imposes only a low overhead}
		\visible<7->{\item[$-$]	partitioning required (e.g. manual selection of a partitioning strategy---\textit{called routing rule})}
		\visible<8->{\item[$-$]	partitioning is sensitive to the workload}
		\visible<9->{\item[$-$]	physical partitioning requires expensive repartitioning when the workload changes}
	\end{itemize}
\end{frame}
...
\end{frame}
 ...
 \begin{frame}
	...
	\begin{itemize}
		\visible<2->{\item	data and associated metadata are physically partitioned \bm{$\rightarrow$} Shared-Nothing}
		\visible<3->{\item	partition-level locking using latches is used (only exclusive mode)}
		\visible<4->{\item[$\rightarrow$]	No concurrency control algorithm required!}
		\visible<5->{\item	single-site transactions run serially on the partition's thread (partition needs to be locked)}
		\visible<6->{\item	multi-site transactions require upfront to lock of all relevant partitions/coordination between threads}
	\end{itemize}
\end{frame}

 \begin{frame}
	\frametitle{Pros \& Cons of PSE I}
	
	\begin{itemize}
		\visible<2->{\item[$+$]	only multi-site transactions cause a thread to access unowned records}
			\begin{itemize}
				\visible<3->{\item[$+$]	each CPU cache usually only contains data of its partition \bm{$\rightarrow$} lower cache pollution}
				\visible<3->{\item[$+$]	partition-lock is usually in the CPU cache \bm{$\rightarrow$} synchronization imposes minor overhead}
				\visible<4->{\item[$+$]	each CPU usually only accesses data of its partitions \bm{$\rightarrow$} fewer data movement between NUMA regions}
				\visible<5->{\item[$\rightarrow$]	no physical synchronization beyond the partition-lock required}
			\end{itemize}
		\visible<6->{\item[$\rightarrow$]	scales linearly for single-site transactions}
		\visible<7->{\item[$-$]	multi-site transactions require the locking of all relevant partitions \bm{$\rightarrow$} decreases concurrency drastically}
		\visible<8->{\item[$-$]	partitioning required (e.g. manual selection of a partitioning strategy---\textit{called routing rule})}
	\end{itemize}
\end{frame}

 \begin{frame}
	\frametitle{Pros \& Cons of PSE II}
	
	\begin{itemize}
		\visible<2->{\item[$-$]	partitioning is sensitive to the workload}
		\visible<3->{\item[$-$]	physical partitioning requires expensive repartitioning when the workload changes}
		\visible<4->{\item[$-$]	coordination of multi-site transactions required}
	\end{itemize}
\end{frame}
