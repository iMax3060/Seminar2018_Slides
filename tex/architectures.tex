...
\begin{frame}
	...
	\begin{itemize}
		\visible<2->{\item	metadata (incl. locks) are not partitioned}
		\visible<3->{\item[$\rightarrow$]	physical synchronization (latches, atomics) required}
		\visible<4->{\item	data and indices are not partitioned}
...
\begin{frame}[fragile]
	...
	\begin{itemize}
		\visible<2->{\item[$+$]	no partitioning required (e.g. manual selection of a strategy)}
		\visible<3->{\item[$+$]	partitioning would be sensitive to the workload}
		\visible<4->{\item[$+$]	changed workloads would require repartitioning to benefit from partitioning}
...
\begin{frame}
	...
	\begin{itemize}
		...
		\visible<8->{\item[$+$]	intra-transaction parallelism could be exploited for multi-site transactions}
	\end{itemize}
...
\begin{frame}
	...
	\begin{itemize}
		\visible<2->{\item[$-$]	partitioning required (e.g. manual selection of a partitioning strategy---\textit{called routing rule})}
		\visible<3->{\item[$-$]	partitioning is sensitive to the workload}
		\visible<4->{\item[$-$]	multi-site transactions require expensive fiber-migration (probably between NUMA regions)}
		\visible<5->{\item[$-$]	accessed partitions need to be calculated during query analysis for optimal performance \\ \bm{$\rightarrow$} slower accesses with secondary index}
		\visible<6->{\item[$-$]	primary index is shared \\ \bm{$\rightarrow$} centralized latching for inserts/\-deletes still required \\ \bm{$\rightarrow$} some contention on the shared latch}
		\visible<7->{\item[$-$]	centralized deadlock detection still required (for \lstinline{DL\_DETECT})}
	\end{itemize}
\end{frame}
...
\begin{frame}
	...
	\begin{itemize}
		...
		\visible<3->{\item	partition-level locking using latches (only exclusive mode)}
...
\begin{frame}
	...
	\begin{itemize}
		\visible<2->{\item[$-$]	multi-site transactions require locking of all relevant partitions \\ \bm{$\rightarrow$} decreases concurrency drastically}
		\visible<3->{\item[$-$]	partitioning required (e.g. manual selection of a partitioning strategy---\textit{called routing rule})}
		\visible<4->{\item[$-$]	partitioning is sensitive to the workload}
		\visible<5->{\item[$-$]	physical partitioning requires expensive repartitioning when the workload changes}
		\visible<6->{\item[$-$]	coordination of multi-site transactions required}
	\end{itemize}
\end{frame}
